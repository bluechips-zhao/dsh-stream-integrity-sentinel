import { closeToolBlock, createToolBlock, acceptToolDelta, toolBlockFromEnd, validateCompleteTool } from './tool-block.js';
...
function indexOf(chunk) {
    const index = chunk.index;
    return Number.isSafeInteger(index) && index >= 0 ? index : undefined;
}
function shape(chunk) {
...
function validReplay(value, blockCount) {
    if (!isObject(value) || !Object.prototype.hasOwnProperty.call(value, 'response'))
        return false;
    if (value.blocks !== undefined && (!Array.isArray(value.blocks) || value.blocks.length !== blockCount))
        return false;
    return true;
}
function validFailure(value) {
    ...
    if (value.status !== undefined && (!Number.isInteger(value.status) || value.status < 100 || value.status > 599))
        return false;
    if (value.providerRetryAfterMs !== undefined && (!Number.isFinite(value.providerRetryAfterMs) || value.providerRetryAfterMs <= 0))
        return false;
...
export class StreamValidator {
    ...
    get blockCount() { return this.order.length; }
    get sawTool() { return this._sawTool; }
    get violation() { return this.firstViolation; }
    fail(item) {
    ...
    checkCommon(chunk) {
        if (!shape(chunk))
            return violation('SIS_CHUNK_SHAPE');
        if (!CHUNK_TYPES.has(chunk.type))
            return violation('SIS_UNKNOWN_CHUNK', undefined, undefined, String(chunk.type));
        return chunk;
    }
    push(input) {
        this._chunkCount += 1;
        if (this._chunkCount > this.config.maxChunksPerCall)
            return this.fail(violation('SIS_LIMIT_EXCEEDED', undefined, this._chunkCount, 'chunks'));
        const checked = this.checkCommon(input);
        if (!shape(checked))
            return this.fail(checked);
        const chunk = checked;
        if (this.finishSeen) {
            if (chunk.type === 'finish')
                return this.fail(violation('SIS_FINISH_DUPLICATE'));
        ...
        switch (chunk.type) {
            case 'block-start': {
                ...
                if (typeof chunk.blockType !== 'string' || chunk.blockType.length === 0)
                    return this.fail(violation('SIS_CHUNK_SHAPE', index));
                if (this.blocks.has(index))
                    return this.fail(violation('SIS_BLOCK_START_DUPLICATE', index));
                if (this.order.length >= this.config.maxBlocksPerCall)
                    return this.fail(violation('SIS_LIMIT_EXCEEDED', index, this.order.length + 1, 'blocks'));
            ...
            case 'reasoning-delta': {
                ...
                if (block.type !== expected)
                    return this.fail(violation('SIS_BLOCK_TYPE_CONFLICT', index));
                if (block.closed)
                    return this.fail(violation('SIS_STRAGGLER', index));
            ...
            case 'tool-call-delta': {
                ...
                if (block.type !== 'tool-call')
                    return this.fail(violation('SIS_BLOCK_TYPE_CONFLICT', index));
                if (block.closed || block.tool === undefined)
                    return this.fail(violation('SIS_STRAGGLER', index));
                const id = chunk.id;
                if (typeof id === 'string' && id.trim().length > 0 && block.tool.id === undefined && this.ids.has(id))
                    return this.fail(violation('SIS_TOOL_ID_DUPLICATE', index));
                const result = acceptToolDelta(block.tool, chunk, this.config.maxArgumentsBytesPerTool);
                if (result)
                    return this.fail(result);
                if (block.tool.id !== undefined)
                    this.ids.add(block.tool.id);
                this._sawTool = true;
            ...
            case 'block-end': {
                ...
                if (typeof index !== 'number')
                    return this.fail(index);
                if (!isObject(chunk.block) || typeof chunk.block.type !== 'string' || chunk.block.type.length === 0)
                    return this.fail(violation('SIS_CHUNK_SHAPE', index));
                let block = this.blocks.get(index);
                let firstSeenBlock = false;
                if (!block) {
                ...
                if (block.type !== chunk.block.type)
                    return this.fail(violation('SIS_BLOCK_TYPE_CONFLICT', index));
                if (block.closed)
                    return this.fail(violation('SIS_BLOCK_END_DUPLICATE', index));
                if (block.type === 'tool-call') {
            ...
            case 'usage':
                if (this.usageSeen)
                    return this.fail(violation('SIS_USAGE_DUPLICATE'));
                if (!validUsage(chunk.usage))
                    return this.fail(violation('SIS_CHUNK_SHAPE'));
                this.usageSeen = true;
                return { toolRelated: false, firstSeenBlock: false };
            case 'finish': {
                if (!isObject(chunk.reason) || typeof chunk.reason.kind !== 'string' || !FINISH_KINDS.has(chunk.reason.kind))
                    return this.fail(violation('SIS_CHUNK_SHAPE'));
                const kind = chunk.reason.kind;
                if ((kind === 'error' || kind === 'aborted') && !validFailure(chunk.reason.failure))
                    return this.fail(violation('SIS_CHUNK_SHAPE'));
                if (chunk.replayState !== undefined
                    && (kind === 'max-tokens' || kind === 'error' || kind === 'aborted'))
                    return this.fail(violation('SIS_REPLAY_STATE_INVALID'));
                if (chunk.replayState !== undefined && !validReplay(chunk.replayState, this.order.length))
                    return this.fail(violation('SIS_REPLAY_STATE_INVALID'));
                if ((kind === 'stop' || kind === 'tool-calls') && [...this.blocks.values()].some(block => !block.closed))
                    return this.fail(violation('SIS_CHUNK_SHAPE'));
                if (this._sawTool && kind === 'stop')
                    return this.fail(violation('SIS_FINISH_TOOL_MISMATCH'));
                if (kind === 'tool-calls') {
                    const tools = [...this.blocks.values()].filter(block => block.type === 'tool-call');
                    if (tools.length === 0 || tools.some(block => block.tool === undefined || validateCompleteTool(block.tool)))
                        return this.fail(violation('SIS_FINISH_TOOL_MISMATCH'));
                }
    ...
    finish() {
        if (!this.finishSeen)
            return violation('SIS_FINISH_MISSING');
        return undefined;
    }
    summary() {
        return {
