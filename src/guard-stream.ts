...
import type { DiagnosticSink, GuardContext, SentinelConfig, StreamChunk, Violation } from './types.js'
...
function errorFinish(code: 'STREAM_INTEGRITY_VIOLATION' | 'STREAM_INTEGRITY_INTERNAL', item: Violation, context: GuardContext): StreamChunk {
  return {
    type: 'finish',
    reason: {
      kind: 'error',
...
function nonSuccessFinish(chunk: StreamChunk): StreamChunk {
...
async function cleanup(iterator: AsyncIterator<unknown> | undefined): Promise<void> {
  try { await iterator?.return?.() } catch { /* cleanup failure cannot release quarantined data */ }
}
...
export async function* guardStream(
  ...
  let truncated = false
  let observedFirstViolation: Violation | undefined
  let observedViolationCount = 0

  const observeViolation = (item: Violation): void => {
  ...
  const emitObserveSummary = (): void => {
    if (observedFirstViolation !== undefined && observedViolationCount > 1) {
      emitDiagnostic(diagnostics, violation(
        observedFirstViolation.rule,
        observedFirstViolation.index,
  ...
  const fail = async (item: Violation, internal = false): Promise<StreamChunk> => {
    emitDiagnostic(diagnostics, item)
    cleaned = true
  ...
  try {
    ...
    while (true) {
      ...
      if ('violation' in result && result.violation !== undefined) {
        if (config.mode === 'observe') {
          ...
          if (result.violation.rule === 'SIS_LIMIT_EXCEEDED') truncated = true
          yield raw as StreamChunk
          continue
        }
      ...
      if (!quarantined && result.toolRelated) quarantined = true
      if (quarantined) {
    ...
    if (!done && !cleaned) await cleanup(iterator)
    tail.length = 0
  }
}
